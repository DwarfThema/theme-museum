...
const DasomPage: React.FC<ContentsProps> = ({ onMainLoading }) => {
  return (
    <div className="bg-blackbase w-screen h-screen flex justify-center xl:items-center lg:items-end zero:items-end user no-drag">
      <VideoBackGround src="/videos/dasom.mp4" onLoaded={onMainLoading} />
      <div className="w-full h-screen absolute lg:bg-gradient-to-r zero:bg-gradient-to-t from-blackOpa via-blackHardNext  to-transparent no-drag" />
      {/* 본문 */}
      <div className="z-10 xl:w-10/12 zero:w-11/12 xl:mb-0 lg:mb-[100px] zero:mb-[80px] text-white">
        <div>
          ...
          <div className="lg:text-xl lg:mt-2 zero:text-sm zero:mt-0 font-light mt-2 flex items-center">
            ...
            <InstaLink userName="sseeoul" className="m-1 ml-2" width={15} bg />
          </div>
        </div>
...
'기억'은 뇌 과학, 심리학, 철학 등 다양한 분야에서 연구되고 있으며, 그 복잡성과 신비로움으로 인해 계속해서 우리를 매료시키는 주제 중 하나이다. 내가 창작을 하는 행위는 기억의 복잡성을 직접 경험하고 탐구하는 과정이다.`}
        ...
        <div className="xl:mt-7 lg:mt-0 zero:mt-0">
          ...
          <div className="xl:mt-3 lg:mt-3 zero:mt-2 flex ">
            <Link href="https://www.memory-service-centre.com/" target="_blank">
              <button>
                <Image
                  src={Logo}
                  alt="entrance"
                  className="xl:w-[185px] zero:w-[130px] h-fit"
                  quality={30}
                />
              </button>
            </Link>
